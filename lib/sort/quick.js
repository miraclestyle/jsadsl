const { swap, less } = require('./util');
const shuffle = require('./shuffle');

const partition = (array, compare, low, high) => {
  let left = low;
  let right = high + 1;
  while (true) {
    while (true) {
      left += 1;
      if (!less(array[left], array[low], compare) || left === high) {
        break;
      }
    }
    while (true) {
      right -= 1;
      if (!less(array[low], array[right], compare) || right === low) {
        break;
      }
    }
    if (left >= right) {
      break;
    }
    swap(array, left, right);
  }
  swap(array, low, right);
  return right;
};

const sort = (array, compare, low = 0, high = array.length) => {
  if (high <= low) {
    return;
  }
  const mid = partition(array, compare, low, high);
  sort(array, compare, low, mid - 1);
  sort(array, compare, mid + 1, high);
};

const quick = (array, compare, low = 0, high = array.length) => {
  shuffle(array, low, high);
  sort(array, compare, low, high - 1);
};

module.exports = quick;
